...
### Отправить файл в Nextcloud public share
...
Команда полностью интерактивная и спрашивает:
...
Локальный файл
Удалённую папку внутри share
Имя файла в Nextcloud
```

Токен и пароль вводятся скрыто. Они не сохраняются в конфиг toolkit и не передаются в аргументах `curl`: временные curl-config файлы создаются в `/run/pve-openvpn/` с root-only правами и удаляются при выходе.

Пример назначения:

```text
Nextcloud: nc.lab.example
Folder:    backups/openvpn
File:      /etc/openvpn/server/server.conf
Remote:    /backups/openvpn/server.conf
```

Для публичного WebDAV используется endpoint:

```text
https://NEXTCLOUD/public.php/dav/files/SHARE_TOKEN/...
```

Скрипт всегда отправляет:
...
Если указана удалённая папка, скрипт может рекурсивно попробовать создать её через WebDAV `MKCOL`. Уже существующая папка (`HTTP 405` на `MKCOL`) считается нормальным случаем. Сама загрузка успешна только при `HTTP 200`, `201` или `204`.

HTTPS обязателен. Скрипт не использует `-k` и не отключает проверку TLS-сертификата. Имена папок/файлов URL-encode’ятся, включая пробелы и UTF-8/кириллицу.
